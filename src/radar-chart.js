var RadarChart = {
  draw: function(id, d, options){
    var cfg = {
      ...
      w: 600,
      h: 600,
      factor: 1,    // box scaling
      factorLegend: .85,
      levels: 3,    // layers of boxes
      maxValue: 0,
      radians: 2 * Math.PI,
      opacityArea: 0.5,
    ...
    drawFrame();
    var maxAxisValues = []; // Store the maximum x, y for this axis
    drawAxis();
    var dataValues = [];
    reCalculatePoints();
    ...
    // Calculate the points to be drawn in the polygon based on the input data
    ...
    function move(dobj, i){
      ...
      // Perform coordinate displacement zeroing to facilitate calculation of slope
      ...
      // The special case of infinite slope
      if(oldX == 0) {
        newY = oldY - d3.event.dy;
        // Check if the range is exceeded
      ...
      else
      {
        ...
        var ratio = newX / oldX; // Compute new values using the concept of similar triangle proportions
      ...
      // Recalculate turning points of polygons
      reCalculatePoints();
      // redraw polygon
      drawPoly();
    }
